...
if ( empty( $hours_posts ) ) {
    // Fallback static hours + note + title
    $hours = array(
    ...
    $note  = 'Last reservation 30 minutes before closing';
    $title = 'Opening Hours';
} else {
    $hours_id = $hours_posts[0]->ID;
    $hours = get_post_meta( $hours_id, '_opening_hours', true );
    ...
    $note  = get_post_meta( $hours_id, '_opening_hours_note', true );
    $note  = $note ?: 'Last reservation 30 minutes before closing';
