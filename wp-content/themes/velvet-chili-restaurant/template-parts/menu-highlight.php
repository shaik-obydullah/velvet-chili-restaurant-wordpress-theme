<?php
/**
 * Menu Highlight Section
 * Displays a single Menu Feature post with image on left, text on right.
 */

if ( defined( 'VCRC_ACTIVE' ) ) {

    $query = new WP_Query(array(
        'post_type'      => 'vchs_menu_feature',
        'posts_per_page' => 1,
        'post_status'    => 'publish',
    ));

    if ( $query->have_posts() ) {
        $query->the_post();

        $image    = get_the_post_thumbnail_url(get_the_ID(), 'full');
        $title    = get_the_title();

        $subtitle = get_post_meta(get_the_ID(), '_vcrc_menu_subtitle', true);
        $body     = get_post_meta(get_the_ID(), '_vcrc_menu_body', true);

        wp_reset_postdata();
?>
<section class="menu-highlight" id="menu">

    <div class="menu-highlight__image">
        <img src="<?php echo esc_url($image); ?>" alt="<?php echo esc_attr($title); ?>" loading="lazy">
    </div>

    <div class="menu-highlight__text">

        <span class="menu-highlight__kicker">Our Menu</span>

        <h2 class="menu-highlight__title">
            <?php echo esc_html($title); ?>
        </h2>

        <p class="menu-highlight__subtitle">
            <?php echo esc_html($subtitle); ?>
        </p>

        <div class="menu-highlight__body">
            <?php echo wp_kses_post($body); ?>
        </div>

    </div>

</section>
<?php
    }
}
else {
?>
<section class="menu-highlight" id="menu">
    <div class="menu-highlight__image">
        <img src="https://images.unsplash.com/photo-1414235077428-338989a2e8c0?q=80&w=2070&auto=format&fit=crop"
            alt="Velvet Chili menu dishes" loading="lazy">
    </div>

    <div class="menu-highlight__text">
        <span class="menu-highlight__kicker">Our Menu</span>
        <h2 class="menu-highlight__title">Crafted with Fire & Spice</h2>
        <p class="menu-highlight__subtitle">
            From small plates to slow-braised classics
        </p>
        <p class="menu-highlight__body">
            Every dish at Velvet Chili is built around the deep, smoky warmth of
            carefully selected chilies.
        </p>
    </div>
</section>
<?php } ?>